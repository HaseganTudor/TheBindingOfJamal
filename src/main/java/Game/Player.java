...
public class Player extends GameObject {
    ...
    public void shoot(long window, double delta) {
        ...
        if (
            glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS && shootSpeed <= 0
        ) {
        ...
        if (
            glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS && shootSpeed <= 0
        ) {
            Tear tear = new Tear(this);
            tear.position = new Vector3f(position);
            tear.setSize(70);
        ...
        if (
            glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS && shootSpeed <= 0
        ) {
            Tear tear = new Tear(this);
            tear.position = new Vector3f(position);
            tear.setSize(70);
